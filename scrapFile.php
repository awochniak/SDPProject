...
$filenameTxt = 'results/'.$_POST['filename'].'.txt';
$filenameJSON = 'results/'.$_POST['filename'].'.json';


echo '<div class="container">
		<div class="row">
			<div class="col-sm">
				<div class="alert alert-dark trn" role="alert">
...
$html = file_get_html($filenameTxt);
...
foreach($list[0]->find('div.text') as $text) {
	...
	$dayOfArrivalShortcut = (int)date('N', $timestampArrival);
	$dayOfDepartureShortcut = (int)date('N', $timestampDeparture);
	...
	foreach($details[0]->find('span.from') as $detailsFrom){
		...
		$intWeekday =  weekdayToInt($departureHour[0]);
		$daysDifference = $dayOfDepartureShortcut - $intWeekday ;
		$dateOfFlight = date('Y-m-d', strtotime($dateOfDeparture . ' +'.$daysDifference.' day'));
		$tableOfResults[$i]['flightTo'][$k]['dateOfFlight'] = $dateOfFlight;
	...
	foreach($details[1]->find('span.from') as $detailsFrom){
		...
		$intWeekday =  weekdayToInt($departureHour[0]);
		$daysDifference = $dayOfArrivalShortcut - $intWeekday ;
		$dateOfFlight = date('Y-m-d', strtotime($dateOfArrival . ' +'.$daysDifference.' day'));
		$tableOfResults[$i]['flightFrom'][$l]['dateOfFlight'] = $dateOfFlight;
...
$response = json_encode($tableOfResults);

unlink($filenameTxt);
$file = fopen($filenameJSON, 'w'); // otwarcie pliku z uprawieniami do zapisu
